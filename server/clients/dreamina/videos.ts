...
function createSignature(
  ...
  if (!timestamp) throw new Error("x-amz-date header is required for AWS signature");
...
export async function generateVideo(
  ...
  if (!isSupportedVideoModelReqKey(model)) {
    throw new APIException(
      ...
      `不支持的视频模型: ${_model || model}. 支持的模型: ${listSupportedVideoModelReqKeys().join(", ")}`
    );
  }

  const adaptedInput = adaptVideoGenerationInput(model, {
    width,
    height,
    resolution,
    durationSec,
  ...
  logger.info(
    `使用模型: ${_model} 映射模型: ${model} ${adaptedInput.width}x${adaptedInput.height} ` +
    `分辨率: ${adaptedInput.resolution} 时长: ${adaptedInput.durationMs}ms ` +
    `比例: ${adaptedInput.aspectRatio} 输入: ${adaptedInput.inputMediaType}`
  );
  ...
  if (adaptedInput.filePaths.length > 0) {
    let uploadIDs: string[] = [];
    logger.info(`开始上传 ${adaptedInput.filePaths.length} 张图片用于视频生成`);
    
    for (let i = 0; i < adaptedInput.filePaths.length; i++) {
      const filePath = adaptedInput.filePaths[i];
      if (!filePath) {
    ...
    if (adaptedInput.useEndFrame && uploadIDs[1]) {
      end_frame_image = {
        format: "",
        height: adaptedInput.height,
        id: util.uuid(),
