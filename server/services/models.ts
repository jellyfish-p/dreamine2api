...
import { request } from "~~/server/clients/dreamina/core";
import { pickOneSession } from "~~/server/services/pool/auth";
import { listCatalogEntries } from "~~/server/services/pool/model-catalog";
...
const SUPPORTED_MODEL_REQ_KEYS = new Set(listCatalogEntries().map((entry) => entry.model_req_key));
...
function fromScraped(file: ScrapedFile): DreaminaModelItem[] {
  const image = (file.image_models || []).filter((m) => SUPPORTED_MODEL_REQ_KEYS.has(m.model_req_key)).map((m) => ({
    id: m.model_req_key,
  ...
  const video = (file.video_models || []).filter((m) => SUPPORTED_MODEL_REQ_KEYS.has(m.model_req_key)).map((m) => ({
    id: m.model_req_key,
    object: "model" as const,
    owned_by: "dreamina.capcut.com",
...
function parseUpstreamList(list: unknown[], modelType: "image" | "video"): DreaminaModelItem[] {
  ...
  for (const item of list) {
    ...
    if (!SUPPORTED_MODEL_REQ_KEYS.has(key)) continue;
