...
import { resolveCreditProxy, resolveApiProxy } from "~~/server/services/pool/settings";
 ...
 *   commerce-api-sg /commerce/v1/subscription/user_info → vip 等级 / 到期时间
 */
export async function fetchAccountInfo(id: number) {
  ...
  const apiProxy = resolveApiProxy(row.proxy_url);
  const creditProxy = resolveCreditProxy(row.proxy_url);
  const commerceInfo = await getCommerceAccountInfo(row.session_id, { proxyUrl: creditProxy });

  // Passport 信息只作为补充。该端点在部分网络/地区会返回 404, 不能阻断前端“获取类型”。
  let info: Record<string, any> = {};
  try {
    info = await getAccountInfo(row.session_id, { proxyUrl: apiProxy });
  } catch (e: any) {
    logger.warn(`账号 ${id} Passport账户查询失败，继续使用Commerce账户信息: ${e?.message}`);
  }

  const userId = firstString(
    info?.user_id,
    info?.uid,
    getStringByPath(commerceInfo, ["user_id"]),
    getStringByPath(commerceInfo, ["uid"]),
    getStringByPath(commerceInfo, ["user_info", "user_id"]),
    getStringByPath(commerceInfo, ["user_info", "uid"]),
    getStringByPath(commerceInfo, ["account_info", "user_id"]),
    row.user_id
  );
  const userName = firstString(
    info?.name,
    info?.screen_name,
    getStringByPath(commerceInfo, ["name"]),
    getStringByPath(commerceInfo, ["screen_name"]),
    getStringByPath(commerceInfo, ["user_name"]),
    getStringByPath(commerceInfo, ["user_info", "name"]),
    getStringByPath(commerceInfo, ["account_info", "name"]),
    row.user_name
  );
  const storeCountry = firstString(
    info?.store_country,
    getStringByPath(commerceInfo, ["store_country"]),
    getStringByPath(commerceInfo, ["store_country_code"]),
    getStringByPath(commerceInfo, ["user_info", "store_country"]),
    getStringByPath(commerceInfo, ["account_info", "store_country"]),
    row.store_country
  );
  const accountType = deriveAccountType({
    ...
    storeCountry,
    previousType: row.account_type,
  });

  let vipLevel: string | null = row.vip_level;
  let vipExpireAt: string | null = row.vip_expire_at;
  vipLevel = extractVipLevel(commerceInfo) ?? vipLevel;
  vipExpireAt = extractVipExpire(commerceInfo) ?? vipExpireAt;
  logger.info(`账号 ${id} Commerce账户信息: type=${accountType}, vip=${vipLevel}, expire=${vipExpireAt}`);

  const mergedInfo = JSON.stringify({ account: info, commerce_account: commerceInfo }).slice(0, 8000);
...
function extractVipLevel(sub: any): string | null {
  ...
  const candidates = [
    ...
    sub.vip_levels?.[0]?.level,
    sub.auto_renew_plans?.[0]?.level,
  ];
...
export async function refreshAllCredits() {
  ...
  for (const a of accounts) {
    try {
      ...
      results.push({ ok: true, ...r, id: a.id });
    } catch (e: any) {
...
export function toPublicAccount(row: PoolAccountRow) {
  return {
    ...
    last_vip_credit: row.last_vip_credit,
    last_purchase_credit: row.last_purchase_credit,
