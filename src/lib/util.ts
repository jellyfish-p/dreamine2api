...
const util = {
  ...
  buildDataBASE64(type: string, ext: string, buffer: Buffer) {
  ...
  isReadStream(value: any): boolean {
    return (
      value &&
  ...
  isWriteStream(value: any): boolean {
    return (
      value &&
  ...
  isDataString(value: any): boolean {
    return /^(base64|json):/.test(value);
  },

  isStringNumber(value: any) {
  ...
  isAsyncFunction(value: any) {
    return Object.prototype.toString.call(value) === "[object AsyncFunction]";
  },

  async isAPNG(filePath: string) {
    let head: Buffer;
    ...
    readStream.once("data", (data: Buffer) => (head = data));
    await readPromise;
  ...
  millisecondsToHmss(milliseconds: any) {
  ...
  millisecondsToTimeString(milliseconds: number) {
