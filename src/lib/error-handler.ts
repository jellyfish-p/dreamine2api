 ...
 * Dreamina API错误响应接口
 */
export interface DreaminaErrorResponse {
  ret: string;
  errmsg: string;
  data?: any;
  historyId?: string;
...
/**
 * 统一的Dreamina API错误处理器
 */
export class DreaminaErrorHandler {
  
  /**
   * 处理Dreamina API响应错误
   */
  static handleApiResponse(
    response: DreaminaErrorResponse, 
  ...
  static handleNetworkError(
    ...
    if (error.code === 'ENOTFOUND') {
      throw new APIException(EX.API_REQUEST_FAILED, `[网络错误]: 无法连接到Dreamina服务器，请检查网络连接`);
    }
    
    if (error.response?.status >= 500) {
      throw new APIException(EX.API_REQUEST_FAILED, `[服务器错误]: Dreamina服务器暂时不可用 (${error.response.status})`);
    }
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (attempt < maxRetries) {
          logger.warn(`${context}失败 (尝试 ${attempt + 1}/${maxRetries + 1}): ${(error as Error).message}`);
 ...
 * 便捷的错误处理函数
 */
export const handleDreaminaError = DreaminaErrorHandler.handleApiResponse;
export const handleNetworkError = DreaminaErrorHandler.handleNetworkError;
export const handlePollingTimeout = DreaminaErrorHandler.handlePollingTimeout;
export const handleGenerationFailure = DreaminaErrorHandler.handleGenerationFailure;
export const withRetry = DreaminaErrorHandler.withRetry;
