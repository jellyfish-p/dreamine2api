...
import { resolveApiProxy, resolveCreditProxy } from "@/lib/pool/settings.ts";
...
/**
 * 获取订阅/会员信息 (海外站 /commerce/v1/subscription/user_info)
 *
 * 返回 vip 等级、到期时间、订阅状态等
 * 注意: 该接口用 commerce-api-sg.capcut.com (新加坡区), appid=348188
 */
export async function getSubscriptionInfo(refreshToken: string, proxyOpts: RequestProxyOptions = {}) {
  const token = await acquireToken(refreshToken);
  const deviceTime = util.unixTimestamp();
